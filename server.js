...
const authenticateFirebaseToken = async (req, res, next) => {
    ...
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
        return res.status(401).send({ message: 'Yetkilendirme tokenı eksik veya hatalı.' });
    }
...
app.post('/api/create-payment', authenticateFirebaseToken, async (req, res) => {
    ...
    try {
        ...
        res.status(200).send({
            ...
            paymentId,
            paymentLink: simulatedPaymentLink
        });
    } catch (error) {
...
// 2. Ödeme sağlayıcısından gelen webhook (simülasyon)
app.post('/api/simulate-payment-webhook', async (req, res) => {
    ...
    try {
        await db.runTransaction(async (transaction) => {
            ...
            // İşlemin zaten tamamlanmadığından emin olun
...
// 3. Kullanıcının para yükleme geçmişini getirme
