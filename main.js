...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Admin login sayfası için auth kontrolü
        ...
        if (loginForm) {
            loginForm.addEventListener('submit', async function(e) {
                ...
                try {
                  ...
                  // onAuthStateChanged will handle all redirection logic.
                } catch (error) {
        ...
        onAuthStateChanged(auth, async (user) => {
            if (user) {
                showLoader(); // Check user role, show loader
                try {
                    const userDoc = await getDoc(doc(db, "users", user.uid));
                    if (userDoc.exists() && userDoc.data().isAdmin) {
                        ...
                        // Regular user, redirect to the main site index
                        window.location.replace('index.html');
                    }
                } catch (error) {
                    console.error("Admin yetki kontrolü sırasında hata:", error);
                    await auth.signOut(); // Sign out to prevent potential loops
                    showAlert("Kullanıcı yetkisi kontrol edilirken bir hata oluştu.", false);
                    hideLoader();
                }
            } else {
                // No user logged in, just hide loader and stay on the page
                hideLoader();
            }
        });
    }
