...
onSnapshot(doc(db, 'users', user.uid), (docSnapshot) => {
    if(docSnapshot.exists()) {
        ...
        setDoc(doc(db, "users", user.uid), {
            username: user.email.split('@'), email: user.email, balance: 0, isAdmin: false,
...
depositBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        // In a real Papara integration, this call would go to your backend,
        // which then communicates with the Papara API to create a payment link.
        ...
        // --- GERÇEK PAPARA ENTEGRASYONU BURADA YAPILMALIDIR ---
        // Gerçek bir uygulamada, 'paymentLink' Papara'nın ödeme sayfasına yönlendirir:
        // window.location.href = paymentLink;
        // Kullanıcı Papara'nın sitesinde ödemeyi tamamlar.
        // Papara daha sonra sunucunuzun belirlediği webhook URL'sine (örn. /api/papara-webhook)
        // ödeme durumu hakkında bildirim gönderir.
        // Bu simülasyon, ödeme tamamlandıktan sonra belirli bir gecikmeyle
        // '/api/simulate-payment-webhook' endpoint'ini doğrudan çağırır.
        // ---------------------------------------------------

        // SADECE SİMÜLASYON: Belirli bir gecikmeyle simüle edilmiş webhook'u doğrudan çağır
        setTimeout(async () => {
            ...
            const simulatePaymentResponse = await fetch(`${BACKEND_URL}/api/simulate-payment-webhook`, {
                ...
                body: JSON.stringify({ paymentId: paymentId, status: 'success' }) // Demo için her zaman başarıyı simüle et
            });
        ...
        }, 2000); // 2 saniyelik ödeme sürecini simüle et

    } catch (error) {
        console.error("Para yükleme hatası:", error);
        showAlert("Para yüklenirken bir hata oluştu: " + error.message, false);
    } finally {
        // Buton durumunu tekrar etkinleştir. Bakiye için onSnapshot dinleyicisi metni doğal olarak güncelleyecektir.
        depositBtn.disabled = false;
        depositBtn.textContent = "Seçilen Miktarı Yükle";
    }
});
...
code
Code
let username = user.displayName || user.email.split('@');
